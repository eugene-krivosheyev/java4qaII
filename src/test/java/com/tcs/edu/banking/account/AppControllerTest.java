...
public class AppControllerTest {
    ...
    public void shouldCreateAccount(String type) throws IOException {
        var id = Integer.parseInt(
                appController.process(
                        new RoundFormatMessage("CREATE ACCOUNT " + type, Severity.INFO)));

        var report = appController.process(
                new RoundFormatMessage("REPORT", Severity.INFO));

        assertThat(report).containsOnlyOnce(id + ": " + type + " 0.0");
    }

    @Test
    public void shouldTransferFromCreditToCredit() throws IOException {
        var fromId = Integer.parseInt(
                appController.process(
                        new RoundFormatMessage("CREATE ACCOUNT CREDIT", Severity.INFO)));
